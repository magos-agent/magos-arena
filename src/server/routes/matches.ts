...
interface Match {
  id: string;
  gameId: string;
  player1: string; // agent ID
  player2: string; // agent ID
  state: GameState;
  moves: { player: 1 | 2; action: number; timestamp: string }[];
  status: 'pending' | 'active' | 'completed' | 'aborted';
  result?: {
    winner: string | null; // agent ID or null for draw
    ratingChanges: { [agentId: string]: number };
  };
  stakes?: number; // USDC amount
  rake?: number;
  createdAt: string;
  startedAt?: string;
  completedAt?: string;
...
const pendingQueue: string[] = []; // Match IDs waiting for opponent
...
matchesRouter.post('/:matchId/move', async (c) => {
  ...
  const result = makeMove(match.state, action as Column);
  ...
  if (result.state.isGameOver) {
    ...
    if (result.state.winner === 1) winnerId = match.player1;
    else if (result.state.winner === 2) winnerId = match.player2;
    ...
    if (agent1 && agent2) {
      ...
      const { newRatingA, newRatingB } = calculateNewRatings(
        player1, player2, gameResult as 1 | 0.5 | 0
      );
...
matchesRouter.get('/', (c) => {
  ...
  if (agentId) {
    matchList = matchList.filter(m => 
      m.player1 === agentId || m.player2 === agentId
    );
  }
